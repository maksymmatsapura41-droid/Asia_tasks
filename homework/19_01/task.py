# Есть автоматизированный склад. У нас есть Главный Контроллер (основной процесс) и 5 Роботов-манипуляторов (дочерние процессы). Роботы должны выполнить работу и отчитаться в общую память.
...
# Операция counter.value += 1 состоит из трех шагов (чтение, сложение, запись). Чтобы два робота не перезаписали данные друг друга одновременно, оберни этот процесс в lock.
